...
import mockupImageSrc from 'images/smarthm_splash_mock.png';
...
export default function DownloadApp() {
	const defaultData = {
		...
		text: 'Download our app & start your smart home journey.',
		link1Text: 'App Store',
		link1Url: 'http://apple.com',
		link1IconSrc: appleIconImageSrc,
		link2Text: 'Google Play',
		link2Url: 'https://play.google.com/store/apps/details?id=com.indivisible.smarthm',
	...
	return (
		<Container>
			<Content>
				<Row>
					<TextContainer>
						{defaultData.subheading && <Subheading>{defaultData.subheading}</Subheading>}
						<Text>{defaultData.text}</Text>
						<LinksContainer>
							{/* <Link href={link1Url}>
								<img src={link1IconSrc} alt='' />
								<span>{link1Text}</span>
							</Link> */}
							<Link href={defaultData.link2Url}>
								<img src={defaultData.link2IconSrc} alt='' />
								<span>{defaultData.link2Text}</span>
							</Link>
						</LinksContainer>
					</TextContainer>
					<ImageContainer>
						<img src={defaultData.imageSrc} alt='' tw='w-64' />
					</ImageContainer>
				</Row>
